...
// Íconos SVG para las tarjetas (sin cambios)
...
export default async function AdminDashboard() {
  ...
  // Peticiones para obtener las estadísticas (sin cambios en la lógica)
  const { count: productCount } = await supabase.from('products').select('*', { count: 'exact', head: true });
  const { count: categoryCount } = await supabase.from('categories').select('*', { count: 'exact', head: true });
  const { data: stockData } = await supabase.from('products').select('stock');
  
  const totalStock = stockData?.reduce((acc, product) => acc + (product.stock || 0), 0) || 0;
